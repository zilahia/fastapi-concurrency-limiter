...
def aggregate(src: str, dst: str) -> None:
    reqs: defaultdict[str, defaultdict[int, int]] = defaultdict(lambda: defaultdict[int, int](int))
    ...
    all_seconds: list[int] = sorted({sec for v in reqs.values() for sec in v} | set(total_vus))
